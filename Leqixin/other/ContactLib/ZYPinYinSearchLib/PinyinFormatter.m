...
+ (NSString *)convertToneNumber2ToneMarkWithNSString:(NSString *)pinyinStr {
  ...
  if ([lowerCasePinyinStr matchesPatternRegexPattern:@"[a-z]*[1-5]?"]) {
    ...
    long indexOfUnmarkedVowel = defautlIndexValue;
    unichar charA = 'a';
    unichar charE = 'e';
    NSString *ouStr = @"ou";
    ...
    if ([lowerCasePinyinStr matchesPatternRegexPattern:@"[a-z]*[1-5]"]) {
      ...
      long indexOfA = [lowerCasePinyinStr indexOf:charA];
      long indexOfE = [lowerCasePinyinStr indexOf:charE];
      long ouIndex = [lowerCasePinyinStr indexOfString:ouStr];
      if (-1 != indexOfA) {
        ...
        for (long i = [lowerCasePinyinStr length] - 1; i >= 0; i--) {
      ...
      if ((defautlCharValue != unmarkedVowel) && (defautlIndexValue != indexOfUnmarkedVowel)) {
        long rowIndex = [allUnmarkedVowelStr indexOf:unmarkedVowel];
        long columnIndex = tuneNumber - 1;
        long vowelLocation = rowIndex * 5 + columnIndex;
