...
-(void)setTitleArray:(NSArray *)titleArray
{
    ...
    for (int i=0; i<titleArray.count; i++) {
        ...
        btn.tag = 100+i;
        [btn setBackgroundColor:[UIColor colorWithRed:0/255.0 green:170/255.0 blue:238/255.0 alpha:1.0]];
...
//        if(i>0){
//            UIImageView *sepatorLine = [[UIImageView alloc]initWithFrame:CGRectMake(i*_config.itemWidth, 10, 2, height-20)];
//            sepatorLine.image = [UIImage imageNamed:@"icon_line"];
//            [btn addSubview:sepatorLine];
//        }
        if(i==0){
...
//            self.line = [[UIImageView alloc] initWithFrame:CGRectMake(_config.itemWidth*(1-_config.linePercent)/2.0, CGRectGetHeight(self.frame) - _config.lineHieght, _config.itemWidth*_config.linePercent, _config.lineHieght)];
            self.line = [[UIImageView alloc] initWithFrame:CGRectMake(_config.itemWidth*(1-_config.linePercent)/2.0, CGRectGetHeight(self.frame) - _config.lineHieght + 2 * SCREEN_SCALE_H, 12, 12)];
    //        _line.backgroundColor = _config.selectedColor;
//           _line.backgroundColor = [UIColor whiteColor];
            _line.image = [UIImage imageNamed:@"icon_indicate"];
            
            
          [self addSubview:_line];
        }
        if (i != 0 && titleArray.count == 3) {
        
            UIImageView *lineImgView = [[UIImageView alloc] initWithFrame:CGRectMake(width * i, (height - 20) / 2, 1, 20)];
            lineImgView.image = [UIImage imageNamed:@"icon_line"];
